...
因此，骨干网络与任务不是同一层级的概念。一个 ResNet 骨干可以接分类头、检测头或分割解码器；一个 Swin Transformer 也可以作为多种视觉任务的骨干。
...
- **单阶段检测器**直接在特征图上密集预测类别和边界框。YOLO 将检测看成一次前向传播，SSD 使用多尺度特征图预测不同大小的物体，RetinaNet 则通过 focal loss 缓解前景与背景样本极度不平衡的问题。
- **端到端集合预测检测器**将预测视为一个集合，训练时通过二分图匹配，为每个真实目标选择一个预测，其余预测学习为“无目标”。DETR 用这种方式替代传统的候选区域生成与非极大值抑制（NMS，即去除重复检测框）的流程，但原始版本收敛较慢。后续 Deformable DETR、DINO detector 和 RT-DETR 分别从多尺度可变形注意力、去噪训练和实时推理等方向改进。
...
卷积通过局部连接和参数共享引入适合图像的结构假设；全局自注意力则允许远距离位置直接交互。步幅为 $1$、忽略边界影响时，卷积具有平移等变性，即输入平移后特征图相应平移；下采样和边界处理会限制这一性质。现代模型可以结合卷积与注意力，并根据数据规模、分辨率和部署硬件进行取舍。ConvNeXt 系列表明，经过现代化的架构设计和训练方法，纯卷积网络仍然具有很强的竞争力；ConvNeXt V2 进一步将卷积网络与掩码自编码器（masked autoencoder）预训练结合。
...
表中的方法并非同一层级：例如 DEIM（2025）是改善 DETR 匹配与收敛的训练框架，可以与 D-FINE 等检测器结合使用。所谓“前沿”涉及训练方法、网络结构和任务设置的共同变化，并非单一模型的版本升级。
...
本章先从卷积、池化、填充、步幅和感受野开始，理解 CNN 为什么适合图像的局部结构；随后用 LeNet、AlexNet 和 ResNet 观察骨干网络的演化。接着分别学习：
...
> 总结：视觉模型可以按“骨干网络—多尺度特征—任务头”理解，视觉任务则按输出粒度分为整图分类、物体级检测和像素级分割。经典 CNN 解决了局部特征提取和深层优化问题；Transformer、多模态预训练与可提示基础模型进一步把固定任务模型扩展为可迁移、开放词汇和跨图像—视频的视觉系统。
